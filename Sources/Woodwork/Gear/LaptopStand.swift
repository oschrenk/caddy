...
public struct LaptopStand: Shape3D {
    ...
    public let slotLength = 150.0

    // Fin geometry (profile in xz, extruded along y).
    public let finHeight = 92.0
    public let finThickness = 8.0
    public let finTopFillet = 8.0

    // Arch cutout in each fin (only one fits at this length).
    public let archCount = 1
    public let archHeight = 50.0
    public let footWidth = 25.0

    // Base plate.
    public let baseThickness = 7.0
    public let baseOverhang = 10.0       // base extends past fins along x and y
    public let baseCornerRadius = 14.0
    public let baseTopChamfer = 1.5
    ...
    public let slotThicknesses = [MacBookPro16().thickness, MacBookNeo().thickness]
    public let slotClearance = 2.0

    // Silicone pads (slot floors + inner fin faces).
    public let padThickness = 1.5
    public let padLength = 110.0
    public let padFaceHeight = 34.0
    public let padFloorInset = 1.0

    // Rubber feet.
    public let footSize = 12.0
    public let footHeight = 2.0
    public let footInset = 10.0

    // ---- Derived dimensions ----
    public var slotWidths: [Double] { slotThicknesses.map { $0 + 2 * slotClearance } }
    public var finCount: Int { slotThicknesses.count + 1 }

    // Fin center-Y positions (finCount of them), accumulated across the slots.
    public var finYs: [Double] {
    ...
    public var slotCenterYs: [Double] {
        zip(finYs, finYs.dropFirst()).map { ($0 + $1) / 2 }
    }

    public var width: Double { slotLength + 2 * baseOverhang }
    public var depth: Double {
        2 * baseOverhang + Double(finCount) * finThickness + slotWidths.reduce(0, +)
    }
    public var height: Double { footHeight + baseThickness + finHeight }

    public var slotBottomZ: Double { footHeight + baseThickness }
