...
struct CasterPlate: Shape3D {
    let width = 33.1     // short side (x)
    let depth = 39.2     // long side (y)
    let thickness = 1.3
    let cornerRadius = 4.0

    let holeDiameter = 4.0
    // Measured plate-edge → nearest hole edge; add the radius for hole centers.
    let holeGapFromShortEdge = 2.5 // hole-edge gap along the long axis (y)
    let holeGapFromLongEdge = 2.3  // hole-edge gap along the short axis (x)
...
struct Caster: Shape3D {
    let wheelDiameter = 32.1
    let totalHeight = 44.5 // top of base plate → bottom of wheel

    let wheelWidth = 13.0 // measured

    // Estimated (not measured) — refine when known.
    let trail = 6.0            // wheel-axle offset from the swivel axis (x)
    let swivelDiameter = 26.0  // round bearing race under the plate
    let swivelHeight = 7.0
    let forkLegThickness = 3.0
    let forkGap = 1.5          // clearance between wheel face and fork leg
    let axleDiameter = 5.0
...
func casters(at points: [(x: Double, y: Double)]) -> any Geometry3D {
    Union {
        for p in points {
