...
struct PiRack: Shape3D {
    ...
    let boardThickness = 19.9
    let towerThickness = 4.6
    let piBoardDepth = 89.0   // Y extent of the Pi (narrower than the tower)

    var width: Double { 78.4 }
    var depth: Double { 96.1 }
    var height: Double { baseLift + Double(count) * (towerThickness + boardThickness) }

    // Pi back face in rack-local Y (Pi is centered inside the tower's depth).
    var piBackY: Double { (depth - piBoardDepth) / 2 + piBoardDepth }

    // Z position (rack-local, bottom-up) of each Pi board's mid-height.
    var piCenterZs: [Double] {
        (0 ..< count).map { i in
