...
struct WashBottle: Shape3D {
    let bodyDiameter = 51.0
    let bodyHeight = 85.8           // straight cylindrical section
    let bodyTopFillet = 5.0         // rounded shoulder at top of body
    let neckDiameter = 31.2
    let neckHeight = 25.0           // includes the screw cap

    // Delivery tube: tapers linearly along arc-length from cap to spout base.
    let tubeDiameterAtCap = 7.8
    let tubeDiameterAtSpoutBase = 6.1
    let trunkHeight = 45.0          // vertical reach from cap to bend center
    let spoutLength = 63.0          // horizontal reach from bend center to tip
    let bendRadius = 20.0           // centerline radius of the 90° bend

    // Jet tip (continues the taper at the very end).
    let spoutTipDiameter = 3.1
    let spoutTipLength = 15.5
    ...
    var width: Double { bodyDiameter }
    var depth: Double { bodyDiameter / 2 + spoutLength }
    var height: Double {
