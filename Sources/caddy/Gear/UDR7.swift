...
struct UDR7: Shape3D {
    let diameter = 110.0
    let height = 184.1
    let topFillet = 8.0      // rounded top edge

    // 0.96" status display — small dark glass panel, centered in x on the front,
    // in the upper portion of the body.
    let screenWidth = 24.0   // along x
    let screenHeight = 16.0  // along z
    let screenCornerRadius = 2.5
    let screenCenterZ = 150.0
    let screenInset = 2.0    // how far the glass embeds into the curved surface

    // Footprint is square so the round body centers cleanly when placed.
    var width: Double { diameter }
    var depth: Double { diameter }
