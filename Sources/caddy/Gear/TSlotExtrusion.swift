...
struct TSlotExtrusion: Shape3D {
    var length: Double

    let width: Double          // across flats
    let cornerRadius: Double   // outer corner fillet (R2)
    let boreDiameter: Double   // central self-tapping core bore

    let slotOpening: Double    // a — mouth width ("5" in Profile 5)
    let slotWidest: Double     // b — widest internal point
    let slotDepth: Double      // c — face to chamber floor
    let lipThickness: Double   // d — face to undercut ledge

    let lipRadius: Double           // mouth-edge lip fillet
    let mouthRadius: Double         // mouth-to-ledge fillet
    let chamberCornerRadius: Double // outer chamber corner fillet
    let chamberFloorRadius: Double  // chamber floor fillet
    let chamberWallAngle: Angle     // chamber side wall, measured from horizontal
